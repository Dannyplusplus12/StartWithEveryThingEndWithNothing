...
class Clouds:
	def __init__(self, cloud_images, count=8):
		self.clouds = []

		for i in range(count):
			self.clouds.append(Cloud([random.random() * 99999, random.random() * 99999], random.choice(cloud_images), random.random() * 0.3 + 0.3, random.random() * 0.6 + 0.2))
